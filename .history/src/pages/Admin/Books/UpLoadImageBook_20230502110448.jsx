...
const UpLoadImageBook = (props) => {
  ...
  const handleUpLoadFile = async ({ file, onSuccess, onError }) => {
    ...
    console.log(
      "🚀 ~ file: UpLoadImageBook.jsx:21 ~ UpLoadImageBook ~ name:",
      name
    );

    if (name === "slider") {
      setData((data) => [...data, { name: file.name, uid: file.uid }]);
    }
    if (name === "thumbnail") {
      setData([{ name: file.name, uid: file.uid }]);
      let tmpData = [];
      tmpData.push({ name: file.name, uid: file.uid });
      console.log(
        "🚀 ~ file: UpLoadImageBook.jsx:40 ~ handleUpLoadFile ~ { name: file.name, uid: file.uid }:",
